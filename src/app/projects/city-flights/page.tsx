...
function readCityFlightsMDX(): string | null {
  const candidates = [
    path.join(process.cwd(), "src", "app", "projects", "city-flights", "city-flights.mdx"),
    path.join(process.cwd(), "app", "projects", "city-flights", "city-flights.mdx"),
  ];
...
export default function CityFlightsProjectPage() {
  const mdx = readCityFlightsMDX();
      ...
      <div className="mt-6 prose prose-slate max-w-none prose-img:rounded-xl">
        {mdx ? (
          ...
          <p className="text-slate-600">
            (Couldn’t load <code>city-flights.mdx</code> at build time.)
          </p>
        )}
      </div>
    </main>

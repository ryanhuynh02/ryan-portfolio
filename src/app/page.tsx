...
function AboutCollapse() {
  ...
  // scroll back to About when hiding (same behavior you liked)
  const hideToAbout = () => {
    setOpen(false);
  ...
  return (
    <div className="mt-4">
      {/* Top bar kept for visual consistency, but not clickable */}
      <div className="w-full inline-flex items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm cursor-default select-none">
        ...
        {/* rotated to indicate expanded */}
        <ChevronDown className="h-4 w-4 text-slate-600 rotate-180" />
      </div>

      {open && (
        <>
          {/* Expanded content (always shown on first load) */}
